...
Freshwater science is a major center of gravity in this joint meeting. A keyword classifier on session and presentation titles (multi-label, so a single talk can carry more than one tag) produces the following frame distribution:
...
*Data: 1,455 scheduled presentations across 308 session items, scraped from the public ASLO-SIL 2026 schedule and audited 11 May 2026. About 1,400 primary presenters; about 740 institutions. Frame and method percentages are computed by a keyword classifier applied to session names, session descriptions, and presentation titles; the classifier is multi-label and approximate. Country detection is keyword-based on affiliation strings and email-domain TLDs and reaches about 78% coverage. Freshwater stock data: World Bank indicator ER.H2O.INTR.K3, latest available 1990–2022. Full inventory, classifier code, and reproducible analysis at github.com/erika-freeman/aslo2026-conference-landscape.*

*Version note (v7): two changes vs v6. (1) Reframed throughout from deficit language to opportunity language; specific opening clauses rewritten ("Freshwater science is a major center of gravity…" replaces "The SIL half is doing real work…"; "A map for future invitations" replaces "The geographic gap remains"). Title updated to add "invitations" alongside "numbers and verbs." (2) Corrected a denominator bug in the v6 freshwater analysis: the previous world total summed WB indicator records that included both real countries and aggregate groupings, inflating the world total by ~10× and making every country's freshwater share appear 10× smaller than reality. With the corrected denominator (WB "World" aggregate = 42,809 bcm/yr, matching the sum of real-country values), Brazil's freshwater share is about 13.2% (not 1.3%) and its representation index is about 0.10 (not 1.0). Brazil is therefore not the closest-to-parity country — Japan is. The "Brazil only proportional" claim from v6 has been replaced with the corrected facts throughout.*
