"""
Build submission_bundle_v8/ — apply manuscript .docx formatting defaults.

Defaults (per saved preference: feedback_manuscript_docx_defaults.md):
  - Times New Roman 12pt body.
  - 1.5 line spacing on Normal style.
  - Continuous line numbers, starting at 1.
  - Page numbers in footer, centered.
  - 1-inch margins (Word default; left as-is).

Same content as bundle v7 (manuscript v15 / short v7 / inquiry v13).

Outputs:
  G:\...\_ConferenceLandscape\submission_bundle_v8\
  G:\...\submission_package\Freeman_MeetingHighlights_ASLO-SIL_2026_v9.docx
  G:\...\submission_package\Freeman_OptionB_LongForm_v17.docx
"""
...
BUNDLE = CONFLAND / "submission_bundle_v8"
...
# ===========================================================================
# Formatting helpers (line numbers, page numbers, font, spacing)
# ===========================================================================
def apply_manuscript_format(doc, body_font="Times New Roman", body_size_pt=12):
    """Apply the standard manuscript .docx formatting defaults."""

    # Body Normal style: Times New Roman 12pt, 1.5 spacing
    style = doc.styles["Normal"]
    style.font.name = body_font
    style.font.size = Pt(body_size_pt)
    pf = style.paragraph_format
    pf.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    # East Asian / complex script font fallback (helps if Drive Mac/Win mix)
    ...
    # Heading styles: same font, slightly larger, also 1.5 spacing
    for hname, size in [("Heading 1", 16), ("Heading 2", 13)]:
        try:
            ...
            hr.set(qn("w:ascii"), body_font)
            hr.set(qn("w:hAnsi"), body_font)
            hr.set(qn("w:cs"), body_font)
            hr.set(qn("w:eastAsia"), body_font)
            hrPr.append(hr)
        except KeyError:
            pass

    # List Bullet inherits Normal font via the rFonts above; force size too
    ...
    # Line numbers, continuous from 1
    # Remove any existing lnNumType
    ...
    lnNumType.set(qn("w:distance"), "360")  # 0.25 inch in twips
...
# ===========================================================================
# Helpers reused from prior bundles
# ===========================================================================
def p_text(doc, text):
    p = doc.add_paragraph()
    p.add_run(text)
...
PATH_B = PKG / "Freeman_OptionB_LongForm_v17.docx"
...
PATH_A = PKG / "Freeman_MeetingHighlights_ASLO-SIL_2026_v9.docx"
...
# Inquiry email v13 — unchanged from v7 bundle, copy in
shutil.copy(PKG / "presubmission_inquiry_email_v13.txt", BUNDLE / "01_inquiry_email.txt")
...
# README
# ===========================================================================
readme = """SUBMISSION BUNDLE v8  |  L&O Bulletin presubmission inquiry
============================================================
...
Inquiry     : v13 (same body as v7 bundle)
Hero image  : Concept A watershed

WHAT CHANGED SINCE BUNDLE v7
----------------------------
Formatting only, content unchanged. Both docx files now use:
  - Times New Roman 12pt body
  - 1.5 line spacing
  - Continuous line numbers (start at 1, left margin)
  - Page numbers in footer, centered
  - 1-inch margins (Word default)

These match journal-manuscript review conventions and what coauthors
expect when a draft is shared.
...
Canonical archive (submission_package/):
    Freeman_MeetingHighlights_ASLO-SIL_2026_v9.docx   (Option A)
    Freeman_OptionB_LongForm_v17.docx                 (Option B)

Build script:
.../_GitHubRepo/aslo2026-conference-landscape/scripts/21_build_submission_bundle_v8.py
"""
(BUNDLE / "00_README.txt").write_text(readme, encoding="utf-8")

# ===========================================================================
# Sanity check: confirm formatting actually applied to docx
# ===========================================================================
def inspect_docx(path):
    ...
    for run in section.footer.paragraphs[0].runs:
        for el in run._r.iter():
            if el.tag == qn("w:instrText") and el.text and "PAGE" in el.text:
                has_page_num = True
    return {
        "font_name": style.font.name,
        "font_size_pt": style.font.size.pt if style.font.size else None,
        "line_spacing_rule": str(style.paragraph_format.line_spacing_rule),
        "has_line_numbers": ln is not None,
        "line_num_start": ln.get(qn("w:start")) if ln is not None else None,
        "line_num_restart": ln.get(qn("w:restart")) if ln is not None else None,
