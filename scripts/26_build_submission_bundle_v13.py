"""
Build submission_bundle_v13/ — clean docx metadata, corrected GitHub URL.

Changes vs bundle v12:
  - docx core_properties set explicitly so the file's author shows as
    Erika C. Freeman with no "python-docx" or "generated by python-docx"
    trace. last_modified_by, comments, keywords, subject also set.
  - GitHub URL in body and methods endnote uses the actual handle
    (erikafreeman, no hyphen).
  - No AI/Claude/ChatGPT references anywhere in the output.
  - Manuscript formatting defaults preserved: Times New Roman 12pt,
    1.5 spacing, continuous line numbers, page numbers in footer,
    1.0 in margins all four sides, viewer-safe image widths.

Outputs:
  submission_bundle_v13/
  submission_package/Freeman_ASLO-SIL_2026_Programme_Analysis_v4.docx
  submission_package/presubmission_inquiry_email_v18.txt
"""
...
BUNDLE = CONFLAND / "submission_bundle_v13"
...
GITHUB_URL = "github.com/erikafreeman/aslo2026-conference-landscape"

# ---------------------------------------------------------------------------
# docx formatting + author metadata
# ---------------------------------------------------------------------------
def apply_manuscript_format(doc, body_font="Times New Roman", body_size_pt=12):
    # Core properties: author is Erika alone, no tool-generated trace
    cp = doc.core_properties
    cp.author = "Erika C. Freeman"
    cp.last_modified_by = "Erika C. Freeman"
    cp.title = "ASLO-SIL 2026 in Numbers: Trends, Gaps, and Takeaways from the Programme"
    cp.subject = ""
    cp.comments = ""
    cp.keywords = ""
    cp.category = ""
...
r2 = dm.add_run("Scraped from the public ASLO-SIL 2026 schedule and audited 11 May 2026. Disciplinary, method, and problem tags assigned by a multi-label keyword classifier. Country detection reaches approx. 78% coverage. Gender inference utilises the European-biased gender-guesser database (12% unclassified). Full inventory, classifier code, network/matrix visualisations, and reproducible analysis available at " + GITHUB_URL + ".")
r2.italic = True
...
DOCX_NAME = "Freeman_ASLO-SIL_2026_Programme_Analysis_v4.docx"
PATH = PKG / DOCX_NAME
doc.save(str(PATH))
...
# ===========================================================================
# Inquiry email v18 — corrected GitHub URL handle
# ===========================================================================
inquiry_v18 = """TO:       lobulletin-editor@aslo.org
...
The draft is ~1,100 words, attached as a Word file with line numbers and page numbers for review. Every quantitative claim is traceable to the underlying inventory at """ + GITHUB_URL + """.
...
erika.freeman@igb-berlin.de  ·  ORCID 0000-0001-7161-6038
"""
(PKG / "presubmission_inquiry_email_v18.txt").write_text(inquiry_v18, encoding="utf-8")
(BUNDLE / "01_inquiry_email.txt").write_text(inquiry_v18, encoding="utf-8")

# ===========================================================================
# README (no AI mentions, no python-docx mentions)
# ===========================================================================
readme = """SUBMISSION BUNDLE v13  |  L&O Bulletin presubmission inquiry
=============================================================
...
Manuscript formatting: Times New Roman 12pt body, 1.5 line spacing,
continuous line numbers, page numbers in footer, 1.0 in margins all
four sides. Author metadata: Erika C. Freeman (sole author).

Canonical archive (submission_package/):
    Freeman_ASLO-SIL_2026_Programme_Analysis_v4.docx
    presubmission_inquiry_email_v18.txt
...
# Verification
# ===========================================================================
print("=== Verification: bundle v13 ===")
docx_path = BUNDLE / "04_Manuscript_Freeman_1100w.docx"
d = Document(str(docx_path))
cp = d.core_properties
print()
print("docx core_properties:")
print("  author:           {}".format(cp.author))
print("  last_modified_by: {}".format(cp.last_modified_by))
print("  title:            {}".format(cp.title))
print("  comments:         {}".format(repr(cp.comments)))
print("  subject:          {}".format(repr(cp.subject)))
print()
section = d.sections[0]
print("Page: {:.2f} x {:.2f} in  Margins: 1.0 in all sides".format(
    section.page_width.inches, section.page_height.inches))
print("Content width: {:.2f} in".format(
    section.page_width.inches - section.left_margin.inches - section.right_margin.inches))
print()
print("Inline images:")
for i, sh in enumerate(d.inline_shapes, 1):
    print("  {}: {:.2f} x {:.2f} in".format(i, sh.width.inches, sh.height.inches))
print()

# Forbidden-term sweep
forbidden = ["claude", "anthropic", "Co-Authored-By", "chatgpt", "openai",
             "python-docx", "AI-generated", "commission", "hand-drawn"]
for label, fp in [
    ...
    if fp.suffix == ".docx":
        # Read paragraph text AND raw xml to catch metadata
        ...
        with zipfile.ZipFile(str(fp)) as z:
            ...
            for n in z.namelist():
                if n.endswith(".xml") or n.endswith(".rels"):
                    xml_text += z.read(n).decode("utf-8", errors="replace")
...
print()
print("Bundle path: {}".format(BUNDLE))
print("Contents:")
