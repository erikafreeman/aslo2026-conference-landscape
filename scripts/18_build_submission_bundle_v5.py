"""
Build submission_bundle_v5/ — metaphor + claim discipline pass.

What changed since v4 (manuscript v12 -> v13):
  - Metaphor consolidation: dropped "vertebrae" everywhere; closing thought
    now uses "lakes as recurring confluences" and "long-term monitoring as
    a connective method."
  - Gender claim softened: "shows a strong gender-balance signal" rather
    than "has reached" balance. Data line uses "classifiable presenter
    names" / "classifiable organiser names."
  - "country-detected presentations" replaced with "presentations for
    which a country could be detected" throughout.
  - "Four of the five strongest tag-pairs" softened to "among the strongest
    co-occurrences in the program's tag network."
  - Opening-visual caption shortened.
  - Inquiry email bumped to v11 to match.

Outputs:
  G:\...\_ConferenceLandscape\submission_bundle_v5\
  G:\...\submission_package\Freeman_MeetingHighlights_ASLO-SIL_2026_v6.docx
  G:\...\submission_package\Freeman_OptionB_LongForm_v14.docx
  G:\...\submission_package\presubmission_inquiry_email_v11.txt
"""
...
BUNDLE = CONFLAND / "submission_bundle_v5"
...
# Downsampled hero for embedding
...
# ===========================================================================
# OPTION B — Bulletin Article candidate (manuscript v13 long form)
# ===========================================================================
docB = Document()
...
p_text(docB, "Third, lakes are not just the largest category; they are one of the program's main confluences, where microbial ecology, climate change, long-term monitoring, and biogeochemistry repeatedly meet. In a joint ASLO–SIL meeting it is easy to read freshwater dominance as \"limnology won this round.\" It is more interesting to read it as \"lakes are the system through which most of the other questions get asked.\"")
...
p_text(docB, "The program shows a strong gender-balance signal at both presenter and organiser levels: about 53% female-inferred among classifiable presenter names and 56% among classifiable organiser names, using a European-biased name classifier that left about 12% of names unclassified. Within those limits, ASLO-SIL 2026 looks close to balance — a place much of STEM has not reached.")
...
PATH_A = PKG / "Freeman_MeetingHighlights_ASLO-SIL_2026_v6.docx"
...
# Inquiry email v11
# ===========================================================================
inquiry_v11 = """TO:       lobulletin-editor@aslo.org
...
A few of the load-bearing findings, each anchored to the underlying data:
  ...
  • The program shows a strong gender-balance signal at both presenter and organiser levels: about 53% female-inferred among classifiable presenter names and 56% among classifiable organiser names, using a European-biased name classifier that left about 12% of names unclassified.
...
Both candidate drafts are attached so you can pick the format that fits the Bulletin:
  ...
  B · ~2,000-word Bulletin Article with the opening visual and two data figures.
...
erika.freeman@igb-berlin.de  ·  ORCID 0000-0001-7161-6038
"""
(PKG / "presubmission_inquiry_email_v11.txt").write_text(inquiry_v11, encoding="utf-8")
(BUNDLE / "01_inquiry_email.txt").write_text(inquiry_v11, encoding="utf-8")

# ===========================================================================
# README
# ===========================================================================
readme = """SUBMISSION BUNDLE v5 — L&O Bulletin presubmission inquiry
==========================================================
...
Manuscript  : v13 long form / v5 short form
Inquiry     : v11
Hero image  : Concept A watershed

WHAT CHANGED SINCE BUNDLE v4
----------------------------
Four targeted editorial fixes:
- Metaphor consolidation: dropped "vertebrae" everywhere. Closing thought
  now uses "lakes as recurring confluences" and "long-term monitoring as a
  connective method." Mirror + watershed/confluence retained as the
  load-bearing metaphors.
- Gender claim softened: "shows a strong gender-balance signal" (not "has
  reached"); data line phrased as "classifiable presenter names" and
  "classifiable organiser names."
- "country-detected presentations" replaced with "presentations for which a
  country could be detected" throughout the body and figure captions.
- "Four of the five strongest tag-pairs" softened to "among the strongest
  co-occurrences in the program's tag network" pending re-verification of
  the exact top-5 ranking.
- Opening-visual caption shortened to the editor's suggested form.
- Inquiry email v11 mirrors all of the above.
...
01_inquiry_email.txt                            Email body (v11).
...
-----------------------------------------------
   ...
   Bulletin Article   : 3,000-5,000 words --> Option B (~2,000) sits below
...
.../_GitHubRepo/aslo2026-conference-landscape/manuscripts/
    01_member_narrative_1500w_v13.md   (Option B source - current)
    02_meeting_highlights_500w_v5.md   (Option A source - current)
    VERSION_NOTES.md                   (audit trail through v13)

Build script:
.../_GitHubRepo/aslo2026-conference-landscape/scripts/18_build_submission_bundle_v5.py

Canonical archive:
.../_ConferenceLandscape/submission_package/
    Freeman_MeetingHighlights_ASLO-SIL_2026_v6.docx   (Option A)
    Freeman_OptionB_LongForm_v14.docx                 (Option B)
    presubmission_inquiry_email_v11.txt
...
# ===========================================================================
# Sanity-check: ensure no "vertebrae" / "country-detected" remained
# ===========================================================================
forbidden = ["vertebrae", "country-detected"]
for label, p in [
    ("Option A", BUNDLE / "04_OptionA_short_form_900w.docx"),
    ("Option B", BUNDLE / "05_OptionB_long_form_2000w.docx"),
]:
    d = Document(str(p))
    full_text = "\n".join(par.text for par in d.paragraphs).lower()
    hits = [w for w in forbidden if w in full_text]
    if hits:
        print("WARNING: {} still contains: {}".format(label, hits))
    else:
        print("OK: {} clean of {}.".format(label, forbidden))
...
print("Option B docx total word count: {}".format(body_wc(BUNDLE / "05_OptionB_long_form_2000w.docx")))
